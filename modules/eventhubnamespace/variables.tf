...
variable "event_hub_namespace_dedicated_cluster_id" {
  ...
  default     = null
  validation {
    condition     = var.event_hub_namespace_dedicated_cluster_id == null || length(split("/", var.event_hub_namespace_dedicated_cluster_id)) == 9
...
variable "event_hubs" {
  ...
  validation {
    condition = alltrue([
      ...
      length([for partitimessage_retentionon_count in values(var.event_hubs)[*].message_retention : message_retention if message_retention >= 1 && message_retention <= 90]) <= 0,
    ])
