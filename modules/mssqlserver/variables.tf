...
variable "mssql_server_name" {
  ...
  sensitive   = false
  validation {
    condition     = length(var.mssql_server_name) >= 2
...
variable "mssql_server_connection_policy" {
  ...
  default     = "Default"
  validation {
    condition     = contains(["Default", "Proxy", "Redirect"], var.mssql_server_connection_policy)
    error_message = "Please specify a valid connection policy."
  }
}
...
variable "private_dns_zone_id_sqlserver" {
  ...
  default     = ""
  validation {
    condition     = var.private_dns_zone_id_sqlserver == "" || (length(split("/", var.private_dns_zone_id_sqlserver)) == 9 && endswith(var.private_dns_zone_id_sqlserver, "privatelink.database.windows.net"))
