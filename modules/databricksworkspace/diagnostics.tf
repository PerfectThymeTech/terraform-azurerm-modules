resource "azurerm_monitor_diagnostic_setting" "diagnostic_setting_databricks_workspace" {
  ...
  target_resource_id         = azurerm_databricks_workspace.databricks_workspace.id
  ...
  dynamic "enabled_log" {
    iterator = entry
    for_each = data.azurerm_monitor_diagnostic_categories.diagnostic_categories_databricks_workspace.log_category_groups
    content {
  ...
  dynamic "metric" {
    iterator = entry
    for_each = data.azurerm_monitor_diagnostic_categories.diagnostic_categories_databricks_workspace.metrics
    content {
