resource "azurerm_monitor_diagnostic_setting" "diagnostic_setting_eventgrid_namespace" {
  ...
  target_resource_id         = azurerm_eventgrid_namespace.eventgrid_namespace.id
  ...
  dynamic "enabled_log" {
    iterator = entry
    for_each = data.azurerm_monitor_diagnostic_categories.diagnostic_categories_eventgrid_namespace.log_category_groups
    content {
  ...
  dynamic "enabled_metric" {
    iterator = entry
    for_each = data.azurerm_monitor_diagnostic_categories.diagnostic_categories_eventgrid_namespace.metrics
    content {
